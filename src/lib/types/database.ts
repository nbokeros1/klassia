...
export type Unite = {
  ...
  // ── Champs V3 (SCORGIA V7.5 — AYDTE canonical IDs) ───────────────────────
  sequence_id?: string    // stable SequenceBlock.id from AYDTE — absent on V1/V2
  unit_id?: string        // higher-level unit grouping ID (V7.5+)
}

// ── V3 — Canonical unit record (above sequence level) ─────────────────────────
// Present in ContenuProgramme.units[] only when schema_version = 'v3'.
// Groups one or more AYDTE sequences into a pedagogical macro-unit (e.g. "Bloc A").

export type UnitV3 = {
  id: string              // stable UUID generated by AYDTE bridge
  numero: number
  titre: string           // thematic label for this grouping
  sequence_ids: string[]  // AYDTE SequenceBlock IDs contained in this unit
  outcome_ids: string[]   // NormalizedOutcome IDs covered by this unit
}
...
export type ContenuProgramme = {
  ...
  // 'v3' = generated with AYDTE scaffold; unites carry sequence_id; units[] present
  // absent = legacy programme created before V7.4.3
  schema_version?: 'v1' | 'v2' | 'v3'
  // ── V3 (SCORGIA V7.5) — canonical unit records ────────────────────────────
  units?: UnitV3[]   // macro-units grouping AYDTE sequences; absent on V1/V2
}
